...
let getHttpVerbLength value =
    ...
    if (value &&& 0xFF_FF_FF_FF_FF_FF_FFUL) == DeleteU64 then
        ...
        let vVerbs = v256.Create(value) &&& v256.Create(0xFF_FF_FF_FFUL, 0xFF_FF_FF_FF_FFUL, 0xFF_FF_FF_FF_FF_FFUL, 0xFF_FF_FF_FF_FF_FF_FF_FFUL)

        let vMask1 = Avx2.CompareEqual(vVerbs, v256.Create(GetU64, PostU64, TraceU64, ConnectU64))
        let vMask2 = Avx2.CompareEqual(vVerbs, v256.Create(PutU64, HeadU64, PatchU64, OptionsU64))
...
type HexU32 =
    ...
    member _.U32ToHexSSE(value: u32) =
        ...
        let shuffled = Ssse3.Shuffle(v128.Create(value).u8, v128.Create(0xF0_02_F0_02_F0_03_F0_03UL, 0xF0_00_F0_00_F0_01_F0_01UL).u8)

        // [ 00D4, 00D4, 00C3, 00C3, 00B2, 00B2, 00A1, 00A1 ]
        // [ D400, 4000, C300, 3000, B200, 2000, A100, 1000 ]
        let shiftedLeft = Sse2.MultiplyLow(shuffled.u16, v128.Create(0x1000_0100_1000_0100UL).u16)
        ...
        let mask = Sse2.CompareGreaterThan(shiftedRight.i16, v128.Create(9s))
        let diff = mask.u16 &&& v128.Create(7us)
        let mutable result = shiftedRight + diff + v128.Create(48us)
    ...
    member _.U32ToHexVector(value: u32) =
        ...
        // [ A1, B2, C3, D4, A1, B2, C3, D4, A1, B2, C3, D4, A1, B2, C3, D4 ]
        // [ D4, 00, D4, 00, C3, 00, C3, 00, B2, 00, B2, 00, A1, 00, A1, 00 ]
        let bytes = v128.Shuffle(v128.Create(value).u8, v128.Create(0xF0_02_F0_02_F0_03_F0_03UL, 0xF0_00_F0_00_F0_01_F0_01UL).u8)

        // [ 00D4, 00D4, 00C3, 00C3, 00B2, 00B2, 00A1, 00A1 ]
        // [ D400, 4000, C300, 3000, B200, 2000, A100, 1000 ]
        // [ 000D, 0004, 000C, 0003, 000B, 0002, 000A, 0001 ]
        let nibbles = (bytes.u16 * v128.Create(0x1000_0100_1000_0100UL).u16) >>> 12

        // 9 - 57, 10 - "58" => A - 65, 7 - diff
        let diff = v128.GreaterThan(nibbles.u16, v128.Create(9us)) &&& v128.Create(7us)
        let mutable result = nibbles + diff + v128.Create(48us)
    ...
    member _.U64ToHexVector(value: u64) =
        ...
        // [ A1, B2, C3, D4, A1, B2, C3, D4, A1, B2, C3, D4, A1, B2, C3, D4 ]
        // [ D4, 00, D4, 00, C3, 00, C3, 00, B2, 00, B2, 00, A1, 00, A1, 00 ]
        let bytes = v256.Shuffle(v256.Create(value).u8, v256.Create(0xF0_06_F0_06_F0_07_F0_07UL, 0xF0_04_F0_04_F0_05_F0_05UL, 0xF0_02_F0_02_F0_03_F0_03UL, 0xF0_00_F0_00_F0_01_F0_01UL).u8)

        // [ 00D4, 00D4, 00C3, 00C3, 00B2, 00B2, 00A1, 00A1 ]
        // [ D400, 4000, C300, 3000, B200, 2000, A100, 1000 ]
        // [ 000D, 0004, 000C, 0003, 000B, 0002, 000A, 0001 ]
        let nibbles = (bytes.u16 * v256.Create(0x1000_0100_1000_0100UL).u16) >>> 12

        // 9 - 57, 10 - "58" => A - 65, 7 - diff
        let diff = v256.GreaterThan(nibbles.u16, v256.Create(9us)) &&& v256.Create(7us)
        let mutable result = nibbles + diff + v256.Create(48us)
    ...
    member _.U64ToHexVectorLookup(value: u64) =
        ...
        let lookup = v256.Create(48us, 49us, 50us, 51us, 52us, 53us, 54us, 55us, 56us, 57us, 65us, 66us, 67us, 68us, 69us, 70us)
