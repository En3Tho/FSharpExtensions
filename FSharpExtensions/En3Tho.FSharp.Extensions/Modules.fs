...
module Option =
    let someObj = Some()
    let inline iterAsync ([<InlineIfLambda>] f) opt =
        match opt with
        | Some value -> async.Bind(value, f)
        | None -> async.Zero()
    let inline mapAsync ([<InlineIfLambda>] f) opt = opt |> Option.map f |> async.Return
    let inline bindAsync ([<InlineIfLambda>] f) opt = opt |> Option.bind f |> async.Return
...
module ValueOption =
    ...
    let inline mapAsync ([<InlineIfLambda>] f) opt = opt |> ValueOption.map f |> async.Return
    let inline bindAsync ([<InlineIfLambda>] f) opt = opt |> ValueOption.bind f |> async.Return
...
module String =
    ...
    let [<return: Struct>] inline (|Contains|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.Ordinal) >= 0 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsIgnoreCase|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.OrdinalIgnoreCase) >= 0 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsCurrentCulture|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.CurrentCulture) >= 0 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsCurrentCultureIgnoreCase|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.CurrentCultureIgnoreCase) >= 0 |> ValueOption.ofBool
    ...
    let [<return: Struct>] inline (|ContainsNotCurrentCulture|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.CurrentCulture) = -1 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsNotCurrentCultureIgnoreCase|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.CurrentCultureIgnoreCase) = -1 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsNotInvariantCulture|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.InvariantCulture) = -1 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsNotInvariantCultureIgnoreCase|_|) (pattern: string) (str: string) = str.IndexOf(pattern, StringComparison.InvariantCultureIgnoreCase) = -1 |> ValueOption.ofBool
    let [<return: Struct>] inline (|ContainsNotChar|_|) (pattern: char) (str: string) =
        str.IndexOf(pattern) = -1 |> ValueOption.ofBool
...
module Array =
    ...
    let inline defaultWith ([<InlineIfLambda>] defThunk) arr = if isNullOrEmpty arr then defThunk() else arr
...
module Seq =
    ...
    let inline intersectBy<'a, 'b when 'b: equality> ([<InlineIfLambda>] map: 'a -> 'b) left right = Enumerable.Intersect(left, right, EqualityComparer<'a>.Create map)
    ...
    let inline toLookup ([<InlineIfLambda>] keySelector: 'a -> 'b) ([<InlineIfLambda>] elementSelector: 'a -> 'c) seq = Enumerable.ToLookup(seq, keySelector, elementSelector)
...
module Func =
    let inline fromFun ([<InlineIfLambda>] f) = Func<'a, 'b> f
    let inline fromFun2 ([<InlineIfLambda>] f) = Func<'a, 'b, 'c> f
    let inline fromFun3 ([<InlineIfLambda>] f) = Func<'a, 'b, 'c, 'd> f

module Action =
    let inline fromFun ([<InlineIfLambda>] f) = Action<'a> f
    let inline fromFun2 ([<InlineIfLambda>] f) = Action<'a, 'b> f
    let inline fromFun3 ([<InlineIfLambda>] f) = Action<'a, 'b, 'c> f
